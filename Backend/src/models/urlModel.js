...
class UrlModel{
    ...
    static async create({originalUrl, shortUrl, customUrl, userId, title, qrCode }){
        const query =  `
            ...
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING id, original_url, short_url, custom_url, user_id, title, created_at
        `;
    ...
    static async findByUserId(userId){
        ...
        const query = `
            ...
            u.created_at, COUNT(v.id) as visit_count FROM urls u LEFT JOIN visits v ON u.id = v.url_id
            WHERE u.user_id = $1
            GROUP BY u.id
